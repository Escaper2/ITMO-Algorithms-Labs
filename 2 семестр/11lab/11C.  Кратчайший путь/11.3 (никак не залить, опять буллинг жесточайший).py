...
def dinput(temp=None):
    ...
    [adjencyDict[temp[1] - 1, temp[0] - 1].append(temp[2]) for temp in temp]
...
def ford():
    cnt = 0
    tmp = False
    while not tmp and cnt < countOfVertices:
        tmp = True
        cnt = cnt + 1
        for j,c in adjencyDict.keys():
            if anArr[j][0] < math.inf:
                if anArr[j][0] + adjencyDict[j,c][0] < anArr[c][0]:
                    anArr[c][0] = anArr[j][0] + adjencyDict[j,c][0]
                    tmp = False
...
if __name__ == "__main__":
    with open("pathbgep.in", "r", encoding='utf-8') as file:
        with open("pathbgep.out","w",encoding="utf-8") as output:
            countString = file.readline().split()
            countOfVertices = int(countString[0])
