...
def dinput():
    [matrix.append(list(map(int,file.readline().split()))) for i in range(countOfVertices)]
...
def prim():
    ...
    for i in range(countOfVertices):
        ...
        if anArr[a] == math.inf:
            ...
            exit()
        for j in range(countOfVertices):
            if (anArr[a][0] + matrix[a][j] < anArr[j][0] and (matrix[a][j] != -1)):
...
with open("pathmgep.in", "r", encoding='utf-8') as file:
    with open("pathmgep.out","w",encoding="utf-8") as output:
        countString = file.readline().split()
        countOfVertices = int(countString[0])
        startVert = int(countString[1])
        endVert = int(countString[2])
